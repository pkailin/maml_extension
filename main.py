...
def train_full_model(task_labels_present, basic_maml, num_epochs, sinusoid=False):
    ...
    if task_labels_present == True: 
        ...
        model = train(model, criterion, device, num_epochs, basic_maml=basic_maml, use_task_labels=True, task_labels_present=True, sinusoid=sinusoid)
        
        # Stage 2: Train the entire model including Task Label Generator
        print("\nStarting Stage 2 training...")
        model = train(model, criterion, device, num_epochs, basic_maml=basic_maml, use_task_labels=False, task_labels_present=True, sinusoid=sinusoid)
        ...
        print("\nStarting Training...")
        model = train(model, criterion, device, num_epochs, basic_maml=basic_maml, use_task_labels=False, task_labels_present=False, sinusoid=sinusoid) 
...
# Gaussian 2D Dataset 
# Basic MAML (No Task Labels) 
#model_BasicMAML = train_full_model(task_labels_present=False, basic_maml=True, num_epochs=70000)

# Extensions 
#model_TaskLabel = train_full_model(task_labels_present=True, basic_maml=False, num_epochs=70000)
#model_NoTaskLabel  = train_full_model(task_labels_present=False, basic_maml=False, num_epochs=70000)
